...
class REDGSL(nn.Module):
    ...
    def forward(self, node_embeddings, adj_matrices, timestamp):
        ...
        for t in range(timestamp):
            ...
            local_emb, loss = self.KernelizedMP(node_embeddings[t], adj_matrices[t], self.tau)  # [13095, 32]
        ...
        graph_ssm_output = self.DGSM(z_mp, selective_modeling_adjs)
        # graph_ssm_output = self.DGSM(graph_ssm_input, selective_modeling_adjs)    # [1, 1, 182] --> [1, 1, 182]
        graph_ssm_output = graph_ssm_output.squeeze()
        graph_ssm_output = self.LayerNorm(graph_ssm_output)
        # graph_ssm_output = graph_ssm_output.sum(-1, keepdim=True)
