...
class RolloutStorage(object):
    ...
    def compute_returns(self, next_value, use_gae, gamma, tau, gcn, alpha):
        ...
        features = self.hidden_states.view(total_len, self.hidden_states.size(2)).cuda()  # [total_len, feature_dim]
        edge_index, edge_attr = dense_to_sparse(adj)
        edge_index, edge_attr = edge_index.cuda(), edge_attr.cuda()
        labels = torch.zeros(total_len, dtype=torch.long).cuda()
        ptr = torch.tensor([0, total_len]).cuda()
        train_mask = torch.zeros(total_len, dtype=torch.bool).cuda()
        train_mask[[0, total_len-1]] = True
        ...
        # gcn_phi = torch.exp(gcn(data))
        gcn_phi = torch.exp(gcn([data['x']], [[data['edge_index']]], timestamp=1))
